...
function Locations() {
  ...
  const [portOfLoadings, setPortOfLoadings] = useState([]);
  ...
  useEffect(() => {
    const getPorts = (page) => {
      ...
      PortService.getPorts(page)
        .then((response) => {
          setPortOfLoadings(response.data.data);
    ...
    let restUrl = `?page=${currentPage}&type=1`;
  ...
  const addOrEdit = (port, resetForm) => {
  ...
  const globalSearchClick = () => {
    if (globalSearchValue) {
      ...
      setFilterUrls(`&type=1&port_global_search=${globalSearchValue}`);

      if (filterUrls === `&type=1&port_global_search=${globalSearchValue}`) {
        setSearchLoading(false);
      }
    } else {
      setCurrentPage(1);
