...
function Locations() {
  ...
  useEffect(() => {
    const getLocations = (page) => {
      setLoading(true);
      LocationService.getLocations(page)
        .then((response) => {
  ...
  const globalSearchClick = () => {
    if (globalSearchValue) {
      ...
      setFilterUrls(`&location_global_search=${globalSearchValue}`);

      if (filterUrls === `&location_global_search=${globalSearchValue}`) {
        setSearchLoading(false);
      }
    } else {
      setCurrentPage(1);
            ...
            <div className="card card-height-100">
              <div className="card-header align-items-center d-flex">
                ...
                <div className="from-group" style={{ marginRight: "3px" }}>
                  <input
                    type="text"
                    placeholder="Location Global Search"
